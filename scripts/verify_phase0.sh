...
#   - a token for fridge blackfridge in $TOKEN
#
# Usage:
#   CRYO_DB_DSN=postgresql://cryo@127.0.0.1:5432/cryo \
#   TOKEN=dev-token-blackfridge \
...
TOKEN="${TOKEN:-dev-token-blackfridge}"
...
curl -fsS -X POST "$INGEST_URL/ingest" \
  ...
  -d "{\"fridge\":\"blackfridge\",\"readings\":[{\"ts\":\"$TS\",\"channel\":\"MXC\",\"value\":0.0102,\"unit\":\"K\"}]}"
echo
...
psql "$CRYO_DB_DSN" -c "SELECT ts, fridge, channel, value, unit FROM readings WHERE fridge='blackfridge';"
psql "$CRYO_DB_DSN" -c "SELECT * FROM last_seen WHERE fridge='blackfridge';"
