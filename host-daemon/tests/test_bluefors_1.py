"""Tests for the BlueFors (fridge1) parser, driven by real sample lines.

Covers the format gotchas from samples/fridge1/notes.md: CRLF endings, leading
spaces, day-month-year dates, T-vs-R files, maxigauge multi-gauge lines, blank
(off) gauges, and malformed-line resilience.
"""
...
SAMPLE_DAY = REPO / "samples" / "fridge1" / "26-06-30"


@pytest.fixture
def parser():
    return Bluefors1Parser()
...
def test_maxigauge_skips_blank_off_gauges(parser):
    # Degraded line: gauges off -> blank name, 0 value. Skip them all.
